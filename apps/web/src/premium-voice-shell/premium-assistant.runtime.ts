...
import { AssistantTurnLatencyTracker, compactSourceSummary, sourceCopy, startAnswerAndSourcePresentation } from './premium-assistant-source-presentation';
...
export function bindPremiumAssistantRuntime(){
 ...
 const sourceShell=root.querySelector<HTMLElement>('[data-assistant-sources-shell]');const sourceToggle=root.querySelector<HTMLButtonElement>('[data-assistant-sources-toggle]');const sourceSummary=root.querySelector<HTMLElement>('[data-assistant-sources-summary]');const sourceToggleLabel=root.querySelector<HTMLElement>('[data-assistant-sources-toggle-label]');const sourcePanel=root.querySelector<HTMLElement>('[data-assistant-sources-panel]');const sourceStatus=root.querySelector<HTMLElement>('[data-assistant-sources-status]');const sourceList=root.querySelector<HTMLOListElement>('[data-assistant-sources-list]');const sourceMessages=sourceCopy(language);
 ...
 const client=createPremiumAssistantClient({apiBaseUrl:apiBase,fetch:window.fetch.bind(window),sessionStorage});const history=loadHistory();let answerText='';let activeRequest:AbortController|undefined;let activeSourceRequest:AbortController|undefined;let activeTurnLatency:{turnId:string;tracker:AssistantTurnLatencyTracker}|undefined;let requestSequence=0;let recognition:Recognition|undefined;let cancelRecognitionPromise:(()=>void)|undefined;let activeTurnId:string|undefined;let activeBrowserSpeech:{turnId:string;resolve:()=>void}|undefined;let lastAudioStopReceipt:RuntimeAudioStopReceipt|undefined;let disposed=false;const staleTurnEvents=new Set<string>();const nativeListenerHandles:Array<{remove:()=>Promise<void>}>=[];
 ...
 disposeActivePremiumAssistantRuntime=()=>{
  if(disposed)return;disposed=true;session.off();requestSequence+=1;activeRequest?.abort();activeSourceRequest?.abort();activeRequest=undefined;activeSourceRequest=undefined;activeTurnLatency=undefined;activeTurnId=undefined;
 ...
 sourceToggle?.addEventListener('click',()=>{if(!sourcePanel)return;const expanded=sourceToggle.getAttribute('aria-expanded')==='true';sourceToggle.setAttribute('aria-expanded',String(!expanded));sourcePanel.hidden=expanded;if(sourceToggleLabel)sourceToggleLabel.textContent=expanded?sourceMessages.show:sourceMessages.hide;});
 ...
 async function preemptCurrentTurn(reason:string):Promise<CancellationLease>{
  ...
  activeRequest?.abort();activeSourceRequest?.abort();activeRequest=undefined;activeSourceRequest=undefined;activeTurnLatency=undefined;activeTurnId=undefined;delete runtimeRoot.dataset.activeVoiceTurn;
  ...
  if(isLeaseCurrent({token,sequence})){pendingAction=undefined;if(actionPanel)actionPanel.hidden=true;retryText='';if(retry)retry.hidden=true;answerText='';response.textContent='';panel.hidden=true;resetSourcePresentation();}
 ...
 async function turnOff(){session.off();toggle.setAttribute('aria-pressed','false');requestSequence+=1;activeRequest?.abort();activeSourceRequest?.abort();activeRequest=undefined;activeSourceRequest=undefined;activeTurnLatency=undefined;activeTurnId=undefined;delete runtimeRoot.dataset.activeVoiceTurn;await enqueueVoiceCancellation(async()=>{const results=await Promise.allSettled([stopCapture(),stopSpeaking()]);if(results[1].status==='fulfilled')lastAudioStopReceipt=results[1].value;});}
 ...
 async function processTranscript(confirmedText:string):Promise<boolean>{
  ...
  if(retry)retry.hidden=true;retryText='';const sequence=++requestSequence;const turnId=`turn:${sequence}:${Date.now()}`;activeTurnId=turnId;runtimeRoot.dataset.activeVoiceTurn=turnId;activeRequest?.abort();lastAudioStopReceipt=await enqueueVoiceCancellation(()=>stopSpeaking());if(sequence!==requestSequence||disposed){recordStaleEvent(turnId,'before-model-request');return false;}const controller=new AbortController();activeRequest=controller;let timedOut=false;const timeout=window.setTimeout(()=>{timedOut=true;controller.abort();},18_000);session.transition('UNDERSTANDING');session.markEngineRequest();const turnTracker=new AssistantTurnLatencyTracker(performance.now());activeTurnLatency={turnId,tracker:turnTracker};
  try{
   ...
   if(sequence!==requestSequence||disposed){recordStaleEvent(turnId,'model-response');return false;}session.markEngineResponse(result.timing);turnTracker.acceptServer(result.timing);
   ...
   session.transition('PREPARING');session.markTtsRequest();renderLatency(session.snapshot());const presentation=startAnswerAndSourcePresentation(()=>speak(groundedText,sequence,turnId),()=>presentSources(result,sequence,turnId,turnTracker));void presentation.sources;const spoken=await presentation.audio;if(sequence!==requestSequence||disposed){recordStaleEvent(turnId,'tts-completion');return false;}activeTurnId=undefined;activeTurnLatency=undefined;delete runtimeRoot.dataset.activeVoiceTurn;return spoken;
 ...
 async function presentSources(result:Awaited<ReturnType<typeof client.respond>>,sequence:number,turnId:string,tracker:AssistantTurnLatencyTracker):Promise<void>{
  if(!sourceShell||!sourceSummary||sequence!==requestSequence||disposed)return;
  sourceSummary.textContent=compactSourceSummary(result.sourceTrace,language);sourceShell.hidden=false;if(sourceToggleLabel)sourceToggleLabel.textContent=sourceMessages.show;if(sourceToggle)sourceToggle.setAttribute('aria-expanded','false');if(sourcePanel)sourcePanel.hidden=true;if(sourceStatus)sourceStatus.textContent=result.sourceTrace.status==='LIVE_VERIFICATION_IN_PROGRESS'?sourceMessages.livePending:result.sourceTrace.status==='LIBRARY_ONLY'?sourceMessages.libraryAnswer:result.sourceTrace.status==='NO_VERIFIED_SOURCES'?sourceMessages.noSources:'';tracker.markSourcesVisible(performance.now());recordSourceLatency(turnId,tracker);
  activeSourceRequest?.abort();const sourceController=new AbortController();activeSourceRequest=sourceController;
  try{
   const trace=await client.sources(result.sourceTrace.traceId,{signal:sourceController.signal});if(sequence!==requestSequence||disposed||sourceController.signal.aborted)return;
   if(sourceStatus)sourceStatus.textContent=trace.status==='LIVE_VERIFICATION_IN_PROGRESS'?sourceMessages.livePending:trace.status==='LIBRARY_ONLY'?sourceMessages.libraryAnswer:trace.status==='NO_VERIFIED_SOURCES'?sourceMessages.noSources:'';
   sourceList?.replaceChildren(...trace.sources.map(item=>{const row=document.createElement('li');const title=/^https?:\/\//i.test(item.urlOrIdentifier??'')?document.createElement('a'):document.createElement('strong');title.textContent=item.title;if(title instanceof HTMLAnchorElement&&item.urlOrIdentifier){title.href=item.urlOrIdentifier;title.target='_blank';title.rel='noreferrer noopener';}const metadata=document.createElement('small');const date=new Date(item.timestamp??'');const formattedDate=Number.isNaN(date.getTime())?(item.timestamp??'—'):date.toLocaleDateString(language);metadata.textContent=`${item.retrievalType} · ${item.origin} · ${formattedDate} · ${item.freshness.status}`;row.append(title,metadata);return row;}));
  }catch(error){if(sequence===requestSequence&&!disposed&&!(error instanceof DOMException&&error.name==='AbortError')&&sourceStatus)sourceStatus.textContent=sourceMessages.unavailable;}
  finally{if(activeSourceRequest===sourceController)activeSourceRequest=undefined;}
 }
 function resetSourcePresentation(){if(sourceShell)sourceShell.hidden=true;if(sourcePanel)sourcePanel.hidden=true;if(sourceToggle)sourceToggle.setAttribute('aria-expanded','false');if(sourceSummary)sourceSummary.textContent='';if(sourceToggleLabel)sourceToggleLabel.textContent=sourceMessages.show;if(sourceStatus)sourceStatus.textContent='';sourceList?.replaceChildren();}
 function recordSourceLatency(turnId:string,tracker:AssistantTurnLatencyTracker){const snapshot=tracker.snapshot();persistTelemetry({kind:'assistant-source-latency',turnId,metrics:{'time-to-first-token':snapshot.timeToFirstTokenMs,'time-to-first-audio':snapshot.timeToFirstAudioMs,'answer-complete':snapshot.answerCompleteMs,'sources-visible':snapshot.sourcesVisibleMs},at:Date.now()});}
 ...
 function markAudioStarted(turnId:string,nativeRequestToStartMs?:number){if(turnId!==activeTurnId)return;session.markAudioStart(nativeRequestToStartMs);if(activeTurnLatency?.turnId===turnId){activeTurnLatency.tracker.markAudioStarted(performance.now());recordSourceLatency(turnId,activeTurnLatency.tracker);}session.transition('SPEAKING');const telemetry=session.snapshot();persistTelemetry(telemetry);renderLatency(telemetry);}
