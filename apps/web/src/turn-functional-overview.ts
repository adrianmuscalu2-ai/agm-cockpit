...
import { createTurnProtectedResourceLoader, isTurnRateLimitError, turnProtectedResourceError } from './turn-protected-resource';

const functionalOverviewLoader = createTurnProtectedResourceLoader<TurnFunctionalOverview>({ ttlMs: 60_000 });
let functionalOverviewRetryTimer: ReturnType<typeof setTimeout> | undefined;
...
export async function fetchTurnFunctionalOverview(fetcher: typeof fetch = turnAdminAuthenticatedFetch as typeof fetch) {
  ...
  if (!response.ok || !payload.data) throw turnProtectedResourceError(response, payload.message || `TURN_FUNCTIONAL_OVERVIEW_HTTP_${response.status}`);
  return payload.data;
}

export async function bindTurnFunctionalOverview(
  fetcher: typeof fetch = turnAdminAuthenticatedFetch as typeof fetch,
  administratorAccessVerified = true,
) {
  ...
  if (!administratorAccessVerified) {
    ...
    if (root) renderUnavailable(root, 'AUTH/SESSION FAILURE · Sursa protejată nu este interogată înainte de autorizare.');
    return;
  }
  try {
    const overview = await functionalOverviewLoader.read(() => fetchTurnFunctionalOverview(fetcher));
    const root = document.querySelector<HTMLElement>('[data-turn-functional-overview]');
    if (!root) return;
    renderOverview(root, overview);
    const snapshot = functionalOverviewLoader.snapshot();
    root.dataset.functionalFreshness = snapshot.state;
    if (snapshot.state === 'STALE_CACHE') {
      markStaleProjection(root, snapshot.retryAfterMs);
      scheduleFunctionalOverviewRetry(fetcher, snapshot.retryAfterMs);
    }
  } catch (error) {
    const root = document.querySelector<HTMLElement>('[data-turn-functional-overview]');
    if (!root) return;
    ...
    if (isTurnRateLimitError(error)) scheduleFunctionalOverviewRetry(fetcher, error.retryAfterMs);
  }
}

function scheduleFunctionalOverviewRetry(fetcher: typeof fetch, retryAfterMs: number) {
  if (functionalOverviewRetryTimer !== undefined) return;
  functionalOverviewRetryTimer = setTimeout(() => {
    functionalOverviewRetryTimer = undefined;
    void bindTurnFunctionalOverview(fetcher, true);
  }, Math.max(1_000, retryAfterMs));
}

function markStaleProjection(root: HTMLElement, retryAfterMs: number) {
  const timestamp = root.querySelector<HTMLElement>('[data-functional-generated-at]');
  if (timestamp) timestamp.textContent = `${timestamp.textContent ?? ''} · ULTIMA PROIECȚIE LIVE PĂSTRATĂ · reîncercare în ${Math.ceil(retryAfterMs / 1_000)}s`;
}

export function resetTurnFunctionalOverviewCacheForTest() {
  functionalOverviewLoader.reset();
  if (functionalOverviewRetryTimer !== undefined) clearTimeout(functionalOverviewRetryTimer);
  functionalOverviewRetryTimer = undefined;
}
