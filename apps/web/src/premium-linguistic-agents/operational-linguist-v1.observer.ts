import { turnAdminAuthenticatedFetch } from '../admin-auth';
...
export function refreshOperationalLinguistV1Observer() {
  ...
  activeRequest = observe().finally(() => { activeRequest = undefined; });
...
async function observe() {
  const response = await turnAdminAuthenticatedFetch('/operations/turn/operational-linguists/state', {
    method: 'GET',
    cache: 'no-store',
  ...
  for (const component of body.data?.components ?? []) {
    ...
    recordRuntimeOperationSnapshot(
