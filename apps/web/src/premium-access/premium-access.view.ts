...
const copy = {
  ro: {
    ...
    status: 'Separarea arhitecturală este activă. Verificarea online a entitlement-ului urmează într-o etapă controlată.',
  ...
  de: {
    ...
    status: 'Die architektonische Trennung ist aktiv. Die Online-Prüfung folgt in einer kontrollierten Phase.',
  ...
  en: {
    ...
    status: 'The architectural separation is active. Online entitlement enforcement follows in a controlled stage.',
...
export function renderPremiumAccessView(language: AccessLanguage, escapeHtml: (value: string) => string) {
  ...
  return `<section class="premium-view premium-access-view" aria-labelledby="access-title" data-access-enforcement="session" data-access-state="checking">
    ...
    <form data-access-login class="premium-module" autocomplete="on">
      <div class="premium-module-content"><h2>Autentificare</h2>
        <label>E-mail <input name="email" type="email" autocomplete="username" required /></label>
        <label>Parolă <input name="password" type="password" autocomplete="current-password" required /></label>
        <button class="premium-module-action" type="submit">Verifică accesul</button>
        <button class="premium-back" type="button" data-access-logout>Închide sesiunea</button>
      </div>
    </form>
