...
type NetworkNode = {
  ...
  status: NodeStatus; dependencyState: string; authorityState: { state: string; epoch?: number; fencingToken?: number; providerId?: string; expiresAt?: string };
...
function renderHero(root: HTMLElement, data: Dashboard) {
  ...
  root.querySelector('.agm-control-plane-node')?.classList.add(`status-${statusClass(data.controlPlane.status)}`);
