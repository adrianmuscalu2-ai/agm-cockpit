...
for(const language of basicLanguageCodes){
  ...
  assert.ok(!html.includes('data-assistant-sources'), 'Source trace must not be rendered in the user UI');
...
assert.doesNotMatch(css,/premium-assistant-sources|data-assistant-sources/, 'User CSS must not expose engineering source trace controls');
