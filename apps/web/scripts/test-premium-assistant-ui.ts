...
assert.match(runtime,/history:history\.slice\(-8\)/, 'Premium request context must stay bounded for mobile latency');
assert.match(runtime,/sessionStorage\.setItem/);
assert.match(runtime,/async function interruptAndListen\(\)\{await turnOff\(\);await turnOn\(\);\}/);
assert.match(runtime,/\['UNDERSTANDING','PREPARING','SPEAKING','TRANSCRIBING'\]\.includes\(session\.state\(\)\)/);
...
assert.match(runtime,/session\.markEngineResponse\(\)/, 'Premium latency telemetry must separate the engine response');
...
assert.match(nativeAudio,/public void stopSpeaking[\s\S]*resolvePendingSpeech\(\)/, 'Interrupted Android playback must resolve the pending bridge call');
