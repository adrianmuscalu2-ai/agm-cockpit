...
const cases = [
  ['SunÃƒâ€žÃ†â€™ la +49 7131 555555', 'OPEN_DIALER'],
  ['Deschide navigaÃƒË†Ã¢â‚¬Âºia spre Heilbronn', 'OPEN_MAPS'],
...
assert.equal(selectConversationalCapability(routeCopilotIntent('FotografiazÃƒâ€žÃ†â€™ documentul CMR'), 'FotografiazÃƒâ€žÃ†â€™ documentul CMR', true).status, 'DENIED');
...
for(const forbidden of ['ACCESS_COARSE_LOCATION','ACCESS_BACKGROUND_LOCATION','READ_CALL_LOG','READ_SMS','BIND_NOTIFICATION_LISTENER_SERVICE','READ_EXTERNAL_STORAGE']) assert.equal(manifest.includes(forbidden),false);assert.equal(manifest.includes('android.permission.READ_CONTACTS'),true);
