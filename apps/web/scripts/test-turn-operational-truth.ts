...
assert.match(governanceSource, /turnAdminAuthenticatedFetch/);
assert.match(governanceSource, /AUTH\/SESSION FAILURE/);
assert.match(governanceSource, /nu se deduce DEGRADED sau FAIL din autentificare/);
...
assert.match(commandCenterSource, /data-turn-page="orchestrators"/);
assert.match(commandCenterSource, /data-orchestrator-count="\$\{orchestrators\.length\}"/);
...
assert.match(commandCenterSource, /Premium nu este proiectat/);
assert.match(functionalOverviewSource, /const basicZones = overview\.zones\.filter\(\(zone\) => zone\.tier === 'BASIC'\)/);
assert.doesNotMatch(functionalOverviewSource, /agentGovernanceRegistry/);
...
assert.match(governanceSource, /data\.nodes\.filter\(\(node\) => node\.kind === 'AGENT'\)/);
assert.match(commandCenterSource, /AGM BASIC AGENT GOVERNANCE MAP/);
assert.doesNotMatch(commandCenterSource, /AGM GOVERNANCE IDENTITY MAP|AGM AGENT RUNTIME MAP/);
...
assert.match(productionPreflightSource, /turnAdminAuthenticatedFetch/);
assert.match(productionPreflightSource, /data-production-preflight-status/);
assert.match(secretTelemetrySource, /turnAdminAuthenticatedFetch/);
assert.match(secretTelemetrySource, /AUTH\/SESSION FAILURE/);
...
assert.match(productionPreflightSource, /AUTH\/SESSION FAILURE/);
