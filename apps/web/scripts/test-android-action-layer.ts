...
const contactNounDial = routeAndroidAction('Apelează contactul Mona Vodafone.', null);
assert.equal(contactNounDial.payload?.contactName, 'Mona Vodafone');
assert.equal(contactNounDial.confirmation, 'AGM_REQUIRED');
...
assert.equal(confirmationFor('DIAL'), 'AGM_REQUIRED');
assert.equal(confirmationFor('OPEN_APP'), 'USER_COMMAND');
assert.equal(confirmationFor('MESSENGER_CHAT'), 'ANDROID_TARGET');
...
assert.match(native, /com\.facebook\.orca/);
assert.match(native, /https:\/\/m\.me\//);
...
assert.match(manifest, /com\.facebook\.orca/);
...
assert.match(executor, /effectiveResolution\.action === 'DIAL'/);
assert.match(executor, /resolveStoredPersonalContact/);
assert.match(executor, /resolvePersonalContactAction/);
assert.match(executor, /payload\.contactName && !payload\.value/);
assert.ok(executor.indexOf('resolveStoredPersonalContact') < executor.indexOf('resolveAndroidContactForDial(payload.contactName)'));
...
assert.match(executor, /effectiveResolution\.source === 'ACTIVE_GMAIL_CONTEXT'[\s\S]*\? 'USER_TEXT' : 'PUBLIC'/);
assert.match(gateway, /phase: 'EXECUTION'[\s\S]*pre-action:/);
for (const capability of ['ANDROID_ASSISTANT','OPEN_APP','REMINDER','ALARM','MESSENGER_CHAT','ANDROID_ASSISTANT_SETTINGS']) assert.ok(gateway.includes(capability));
