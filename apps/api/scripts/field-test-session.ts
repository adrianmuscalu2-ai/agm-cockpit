...
async function prepare(ctx:RequestContext){
  ...
  const result={verdict:automaticChecks.every((item)=>item.status==='PASS')?'FIELD_TEST_READY':'FIELD_TEST_NOT_READY',generatedAt:new Date().toISOString(),secretValuesPrinted:false,build:{web:'1.5.0',android:'1.5.0',androidVersionCode:27,apk:'apps/web/public/downloads/AGM-Cockpit-Android-1.5.0.apk'},automaticChecks,operatorChecks,snapshot};
...
async function start(ctx:RequestContext){
  ...
  const snapshot=await runtimeSnapshot(ctx),automaticChecks=checks(snapshot),session={contractVersion:'agm-field-test.v1',sessionId,startedAt:new Date().toISOString(),endedAt:null,status:'ACTIVE',device:process.env.FIELD_TEST_DEVICE??'Android AGM Cockpit 1.5.0',connectivity:process.env.FIELD_TEST_CONNECTIVITY??'OPERATOR_NOT_RECORDED',operatorSafetyConfirmed:process.env.FIELD_TEST_SAFETY_CONFIRMED==='true',secretValuesPrinted:false,baseline:snapshot,automaticChecks};
