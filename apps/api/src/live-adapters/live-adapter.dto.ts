...
export class TollAuthoritySourceDto {@IsString() @MaxLength(180) sourceId!:string;@IsString() @MaxLength(20) jurisdiction!:string;}
export class TollLiveDto extends TrafficLiveDto {@IsString() @MaxLength(240) routeReference!:string;@IsBoolean() tollRequired!:boolean;@ValidateIf((value:TollLiveDto)=>value.tollRequired) @IsIn(TOLL_REQUIRED_REASONS) tollReason?:TollRequiredReason;@IsOptional() @IsDateString() departureTime?:string;@IsOptional() @IsObject() vehicle?:Record<string,unknown>;@IsOptional() @IsArray() @ValidateNested({each:true}) @Type(()=>TollAuthoritySourceDto) authoritySources?:TollAuthoritySourceDto[];@IsOptional() @IsBoolean() authorityScopeConfirmed?:boolean;}
