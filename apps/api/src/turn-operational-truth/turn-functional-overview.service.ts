...
export class TurnFunctionalOverviewService {
  ...
  async snapshot(now = new Date()): Promise<TurnFunctionalOverview> {
    ...
    const zones: TurnFunctionalZone[] = [
      ...
      activityZone('basic.email', 'BASIC', 'Email', conversations, failedMessages > 0 || (gmail?.backlog ?? 0) > 0, latestDate(latestMessage?.statusUpdatedAt, gmail?.updatedAt),
      ...
      activityZone('premium.communications', 'PREMIUM', 'Comunicații', conversations, failedMessages > 0 || (gmail?.backlog ?? 0) > 0, latestDate(latestMessage?.statusUpdatedAt, gmail?.updatedAt),
      ...
      activityZone('premium.copilot', 'PREMIUM', 'Copilot și oportunități', opportunities, staleOpportunities > 0 || opportunityTelemetry.some((item) => item.health !== 'HEALTHY' || item.freshnessStatus !== 'FRESH' || item.backlog > 0) || liveAdapters.some((item) => item.status !== 'HEALTHY'),
        latestDate(...opportunityTelemetry.map((item) => item.lastRunAt), ...liveAdapters.map((item) => item.lastAttemptAt)),
