...
export class AuthController {
  ...
  @Post('refresh') @UseGuards(CsrfOriginGuard) async refresh(@Req() request: Request, @Res({ passthrough: true }) response: Response) { const result = await this.auth.refresh(readCookie(request, COOKIE)); setCookie(response, result.rawRefreshToken); const { rawRefreshToken: _, ...safe } = result; return responseEnvelope(safe); }
