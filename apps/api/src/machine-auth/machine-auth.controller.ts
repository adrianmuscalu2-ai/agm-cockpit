import { Body, Controller, ForbiddenException, Headers, Param, ParseUUIDPipe, Post, Req, UseGuards } from '@nestjs/common';
...
export class DeploymentMachineProvisioningController {
  ...
  async exerciseInspectorFailover(@Req() request: DeploymentRequest, @Headers('x-request-id') requestId?: string) {
    requireAgentRuntimeContinuityAuthority(request.machineProvisioning);
  ...
  async provision(@Body() dto: ProvisionMachineIdentityDto, @Req() request: DeploymentRequest, @Headers('x-request-id') requestId?: string) {
    requireDeploymentProvisioner(request.machineProvisioning);
    const ctx = deploymentContext(request, requestId);
    return responseEnvelope(await this.machines.provision(dto.subject, dto.expiresInDays, ctx), ctx.requestId);
  }
...
function requireAgentRuntimeContinuityAuthority(ctx: RequestContext) {
  if (ctx.roles.length !== 1 || !['DEPLOYMENT_PROVISIONER', 'AGENT_RUNTIME_CONTINUITY'].includes(ctx.roles[0])) {
    throw new ForbiddenException('Agent Runtime continuity authority is required.');
  }
}

function requireDeploymentProvisioner(ctx: RequestContext) {
  if (ctx.roles.length !== 1 || ctx.roles[0] !== 'DEPLOYMENT_PROVISIONER') {
    throw new ForbiddenException('Deployment provisioning authority is required.');
  }
}
