...
export class ExternalDsarIntakeWorkflow{
  constructor(private readonly prisma:any,private readonly notifier:ExternalDsarNotifier,private readonly now=()=>new Date()){}
  async intake(input:{email:string;requestType:string}){
    ...
    const contactHash=hash(email);const duplicates=await this.prisma.$queryRawUnsafe('SELECT id FROM "DataRightsExternalRequest" WHERE "contactHash"=$1 AND status IN (\'IDENTITY_PENDING\',\'VERIFIED\') LIMIT 1',contactHash);if(duplicates.length)return{accepted:true};
