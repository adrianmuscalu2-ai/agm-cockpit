...
- The signing runner now supports owner-authorized one-time DPAPI custody. It writes user-bound ciphertext only after a complete successful signed build, reuses it in memory for later releases, and fails closed for missing/invalid bindings. Parser validation and a dummy in-memory DPAPI round-trip passed; no Production secret was stored during this check.
