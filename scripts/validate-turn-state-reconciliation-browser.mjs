...
try {
  ...
  await page.addInitScript(() => {
    ...
    localStorage.setItem('agm.admin.session', JSON.stringify({ accessToken: 'controlled-turn-admin-session', expiresInSeconds: 600 }));
    ...
    sessionStorage.setItem('agm.turn.incident-journal.v1', JSON.stringify([{ id: 'AGM-MON-ANDROID', updatedAt: '2099-01-01T00:00:00.000Z', status: 'reopened', history: [{ at: '2099-01-01T00:00:00.000Z', action: 'stale-open', actor: 'MON-005', toStatus: 'reopened', note: 'Stale UI copy injected by the reload test.' }] }]));
    localStorage.setItem('agm.turn.telemetry-snapshots.v1', JSON.stringify([['android', { status: 'OFFLINE', checkedAt: '2099-01-01T00:00:00.000Z', changedAt: '2099-01-01T00:00:00.000Z', latencyMs: 1, freshness: 'OFFLINE' }]]));
  });
  await page.route('**/*', async (route) => {
    ...
    if (pathname.endsWith('/authority-control-plane/dashboard')) return json({
      contractVersion: 'AGM-PREMIUM-NETWORK-V1',
      controlPlane: { status: 'PASS', activeExecutiveAuthorities: 1, conflicts: [] },
      nodes: [
        { canonicalId: 'premium.registry-standby', kind: 'AGENT', module: 'operations', ownerId: 'premium.orchestrator', supervisorId: 'premium.orchestrator', scope: 'premium.operations.standby', lifecycleStatus: 'REGISTERED', status: 'STANDBY', statusLabel: 'REGISTERED', statusSource: 'REGISTRY', statusObservedAt: null, dependencyState: 'NO_TELEMETRY', authorityState: { state: 'ADVISORY' }, failoverState: 'STANDBY', telemetry: null, lastRun: null },
        { canonicalId: 'premium.runtime-working', kind: 'AGENT', module: 'operations', ownerId: 'premium.orchestrator', supervisorId: 'premium.orchestrator', scope: 'premium.operations.working', lifecycleStatus: 'REGISTERED', status: 'PASS', statusLabel: 'WORKING', statusSource: 'RUNTIME_EVENT', statusObservedAt: new Date().toISOString(), dependencyState: 'PASS', authorityState: { state: 'ACTIVE' }, failoverState: 'STANDBY', telemetry: null, lastRun: { lifecycle: 'WORKING', occurredAt: new Date().toISOString(), detail: 'Canonical runtime event' } },
        { canonicalId: 'premium.runtime-failed', kind: 'INSPECTOR', module: 'release', ownerId: 'agm.human.product-owner', supervisorId: null, scope: 'premium.release', lifecycleStatus: 'REGISTERED', status: 'FAIL', statusLabel: 'FAILED', statusSource: 'RUNTIME_EVENT', statusObservedAt: new Date().toISOString(), dependencyState: 'FAIL', authorityState: { state: 'ADVISORY' }, failoverState: 'STANDBY', telemetry: null, lastRun: { lifecycle: 'FAILED', occurredAt: new Date().toISOString(), detail: 'Canonical runtime event' } },
      ],
      departments: [{ module: 'operations', nodeCount: 2 }, { module: 'release', nodeCount: 1 }],
      incidents: [], telemetryPolicy: 'OBSERVE_ONLY_NEVER_COMMAND_OR_BLOCK', opportunityIntelligence: { gate: 'GO', reason: 'Controlled reconciliation' },
    });
  ...
  await page.waitForFunction(() => document.querySelectorAll('[data-canonical-agent-id]').length === 3);
  ...
  const audit = await page.evaluate(() => {
    ...
    return {
      ...
      canonicalAgents: [...document.querySelectorAll('[data-canonical-agent-id]')].map((node) => ({ id: node.getAttribute('data-canonical-agent-id'), status: node.getAttribute('data-canonical-status'), className: node.className, text: node.textContent?.replace(/\s+/g, ' ').trim() })),
      controlPlaneClass: document.querySelector('.agm-control-plane-node')?.className ?? '',
      androidActiveIncident: Boolean(document.querySelector('a[href="#incident-AGM-MON-ANDROID"]')),
    };
  });
  const checks = {
    ...
    canonicalPremiumColors: audit.canonicalAgents.length === 3 && audit.canonicalAgents.every((node) => !node.className.includes('status-no-telemetry')) && audit.canonicalAgents.some((node) => node.status === 'STANDBY' && node.text?.includes('REGISTERED')) && audit.canonicalAgents.some((node) => node.status === 'PASS' && node.text?.includes('WORKING')) && audit.canonicalAgents.some((node) => node.status === 'FAIL' && node.text?.includes('FAILED')) && audit.controlPlaneClass.includes('status-pass') && !audit.controlPlaneClass.includes('status-no-telemetry'),
    androidFalseIncidentClosed: !audit.androidActiveIncident,
  };
  ...
  await page.waitForFunction(() => document.querySelector('[data-operation-id="android"] .operation-service-status')?.textContent === 'ONLINE');
  await page.waitForFunction(() => document.querySelectorAll('[data-canonical-agent-id]').length === 3);
  const reloadChecks = await page.evaluate(() => ({
    androidStatus: document.querySelector('[data-operation-id="android"] .operation-service-status')?.textContent,
    androidActiveIncident: Boolean(document.querySelector('a[href="#incident-AGM-MON-ANDROID"]')),
    staleTelemetryRestored: document.querySelector('[data-operation-id="android"]')?.classList.contains('offline') ?? false,
    canonicalStatuses: [...document.querySelectorAll('[data-canonical-agent-id]')].map((node) => node.getAttribute('data-canonical-status')),
  }));
  const reloadPassed = reloadChecks.androidStatus === 'ONLINE' && !reloadChecks.androidActiveIncident && !reloadChecks.staleTelemetryRestored && reloadChecks.canonicalStatuses.join(',') === 'STANDBY,PASS,FAIL';
  results.push({ id: 'reload-canonical-state', status: reloadPassed ? 'PASS' : 'FAIL', detail: reloadChecks });
  if (!reloadPassed) throw new Error(`RELOAD_RECONCILIATION_FAILED:${JSON.stringify(reloadChecks)}`);
