...
let runtimeSnapshot = {
  ...
  inspector: { primaryInspector: 'premium.release-inspector', primaryStatus: 'FAILED', secondaryInspector: 'premium.architecture-inspector', secondaryStatus: 'COMPLETED', activeValidator: 'premium.architecture-inspector', mandateTransferred: true, transferReason: 'PRIMARY_INSPECTOR_FAILED', transferredAt: new Date().toISOString(), lastValidation: new Date().toISOString(), transferEvidenceRef: 'AuthorityAuditJournal:transfer', status: 'PASS', controlStatus: 'TRANSFERRED_TO_SECONDARY', primaryRecovered: false, failoverPreserved: true }, incidents: { open: 1, inspectorFailureIncident: 'incident-primary', controlCoverageIncident: null }, verdict: { controlSystem: 'PASS', overallOperationalState: 'FAIL', agentAccountability: 'FAIL', inspectorFailover: 'PASS', controlCoverage: 'COMPLETE', falseActive: 0, unexplainedDegraded: 0, noTelemetry: 0, failed: 1, mandateNotDemonstrated: 0, primaryRecovered: false, openIncidents: 1, finalAgentRuntimePass: 'FAIL' },
};

function validateRuntimeSnapshot(snapshot) {
  const operational = snapshot?.agents?.filter((agent) => agent.declaredOperational) ?? [];
  const everyAgentAccountable = operational.length > 0 && operational.every((agent) => agent.status === 'ACTIVE' && agent.executable === 'YES' && agent.mandate === 'PROVEN' && agent.validation === 'PROVEN' && agent.executionEvidenceRef);
  const expectedOverall = snapshot?.operationalFleet?.total > 0 && snapshot.operationalFleet.active === snapshot.operationalFleet.total && snapshot.operationalFleet.degraded === 0 && snapshot.operationalFleet.failed === 0 && snapshot.operationalFleet.noTelemetry === 0 && snapshot.operationalFleet.mandateNotDemonstrated === 0 ? 'PASS' : 'FAIL';
