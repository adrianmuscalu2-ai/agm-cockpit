...
const artifactRelative = `${outputRelative}/OWNER_MANUAL_INGEST/RT001-FINAL-FR-SAPN-2026.owner-official.pdf`;
const visualEvidenceRelative = `${outputRelative}/OWNER_MANUAL_INGEST/VISUAL_EVIDENCE_ARTIFACT_03`;
const validationRelative = `${outputRelative}/OWNER_MANUAL_ARTIFACT_03_VALIDATION_REPORT.json`;
...
const artifactId = 'RT001-FINAL-FR-SAPN-2026';
const sourceUrl = 'https://www.groupe.sanef.com/sites/default/files/2026-01/2026_02-Grille-SAPN.pdf';
...
const routeGroups = [
  { id: 'A29_NORD', expression: /AUTOROUTE A29 NORD/gi },
  { id: 'A13_A29_SUD', expression: /AUTOROUTES A13\s*-\s*A29 SUD/gi },
  { id: 'A14', expression: /AUTOROUTE A14/gi },
...
const montessonEntries = (text.match(/PEAGE DE MONTESSON/gi) ?? []).length;
const chambourcyEntries = (text.match(/PEAGE DE CHAMBOURCY/gi) ?? []).length;
...
check('PDF_OPEN_AND_EXTRACT', pdftotextExit === 0 && text.length > 12_000, { pdftotextExit, textLength: text.length }, { pdftotextExit: 0, minimumTextLength: 12_000 });
...
check('OFFICIAL_SAPN_SANEF_IDENTITY_VISUAL', visualEvidence.every((relative) => existsSync(path.join(root, relative))), visualEvidence, '6/6 controlled render captures; cover carries Sanef Paris Normandie identity');
...
check('MONTESSON_ALL_CLASSES', montessonEntries === 5, montessonEntries, 5);
check('CHAMBOURCY_ALL_CLASSES', chambourcyEntries === 5, chambourcyEntries, 5);
check('TARIFF_PAGES_COMPLETE', tariffPages.length === 5 && tariffPages.every((page) => page.trim().length > 2_000), { tariffPageCount: tariffPages.length, textLengths: tariffPages.map((page) => page.trim().length) }, { tariffPageCount: 5, minimumTextLength: 2_000 });
check('VISUAL_RENDER_6_OF_6', visualEvidence.every((relative) => existsSync(path.join(root, relative)) && readFileSync(path.join(root, relative)).length > 180_000), visualEvidence.map((relative) => ({ path: relative, sizeBytes: existsSync(path.join(root, relative)) ? readFileSync(path.join(root, relative)).length : null })), '6/6 rendered pages, no clipped tables on inspection');
...
const report = {
  ...
  provenance: {
    authority: 'SAPN / Sanef group',
    officialDirectPdfUrl: sourceUrl,
    officialHost: 'www.groupe.sanef.com',
    documentIdentity: 'Tarifs de peage 2026 / Sanef Paris Normandie',
    titleMetadata,
    producer,
    creator,
    jurisdiction: 'FR',
  ...
  scope: 'SAPN Paris-Normandie tariff matrices for A29 Nord, A13/A29 Sud and A14, including Montesson and Chambourcy, vehicle classes 1-5',
  ...
  exclusions: 'Pont de Normandie/Tancarville and A150/ALBEA remain separate regimes; their references in this document do not promote or close those regimes.',
  visualEvidence,
...
console.log(JSON.stringify({ verdict: report.verdict, checks: report.checkCount, failed: report.failedCount, pages: report.evidenceCounts.pages, classes: report.evidenceCounts.vehicleClasses, routeGroups: report.evidenceCounts.routeGroups, montesson: report.evidenceCounts.montessonEntries, chambourcy: report.evidenceCounts.chambourcyEntries, sha256: hash, manifest: acquisition.summary }, null, 2));
