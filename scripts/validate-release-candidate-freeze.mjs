...
check('android-release-identity',
  /applicationId\s+["']com\.agm\.cockpit["']/.test(androidBuild)
    && /versionName\s+["']1\.6\.0["']/.test(androidBuild)
    && /versionCode\s+28\b/.test(androidBuild),
  { applicationId: 'com.agm.cockpit', versionName: '1.6.0', versionCode: 28 },
);
...
check('android-current-physical-device', physicalDevices.length === 1 && installedVersion === '1.6.0', {
